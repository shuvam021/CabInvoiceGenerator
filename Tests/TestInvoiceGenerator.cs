...
namespace Tests
{
    ...
    public class TestInvoiceGenerator
    {
        ...
        public void Test_GivenMultipleRides_ShouldReturnAggregateTotalFair()
        {
            ...
            Assert.AreEqual(_appNormal.CalculateFare(payload), new InvoiceSummary(payload.Length, 30));
            Assert.AreEqual(_appPremium.CalculateFare(payload), new InvoiceSummary(payload.Length, 60));
        }
    }
}
